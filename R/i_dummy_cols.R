...
#' @param sep A string to use as separator between column name and unique value.
#'
#' @return
...
i_dummy_cols <- function(.data,
  ...
  data_type <- fastDummies:::check_type(.data)
  ...
  for (col_name in char_cols) {
    ...
    #   data.table::set(.data, j = paste0(col_name, sep, unique_vals), value = 0L)
    .data[, paste0(col_name, sep, unique_vals)] <- 0L
    for (unique_value in unique_vals) {
      data.table::set(.data, i =
                      ...
                      j = paste0(col_name, sep, unique_value), value = 1L)
      ...
      if (!is.na(unique_value)) {
        data.table::set(.data, i =
                        ...
                        j = paste0(col_name, sep, unique_value), value = NA)
      }
      ...
      if (!is.null(split)) {
        ...
        if (is.na(unique_value)) {
          .data[[paste0(col_name, sep, unique_value)]][which(!is.na(.data[[col_name]]))] <- 0
        }
      }
    }
  }
  ...
  .data <- fastDummies:::fix_data_type(.data, data_type)
  if (omit_colname_prefix) {
    if (length(select_columns) == 1) {

      new_col_index <-
        as.logical(rowSums(sapply(unique_vals, function(x)
          grepl(paste0(select_columns, sep, x), names(.data)))))
      names(.data)[new_col_index] <-
        gsub(paste0(select_columns, sep), "", names(.data)[new_col_index])
